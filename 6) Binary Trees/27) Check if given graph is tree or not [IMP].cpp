...
bool Graph::isTree() {
	...
	/* The call to isCyclicUtil serves multiple purposes. It returns true if graph reachable from 
        vertex 0 is cyclic. It also marks all vertices reachable from 0. */
	if (isCyclicUtil(0, visited, -1))
			return false;

	/* If we find a vertex which is not reachable from 0 (not marked by isCyclicUtil(), then we 
        return false */
	for (int u = 0; u < V; u++)
		if (!visited[u])
		return false;
...
int main() {
    ...
    cout<<"\nCheck whether the given Graph is Tree or not? "<<endl;
